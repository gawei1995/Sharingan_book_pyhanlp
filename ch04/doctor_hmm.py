...
def convert_map_to_vector(map,label_index):
    ...
    return JArray(JFloat,v.ndim)(v.tolist()) #numpy 转为Java数组
...
FirstOrderHiddenMarkoveModel = JClass('com.hankcs.hanlp.model.hmm.FirstOrderHiddenMarkovModel')
given_model = FirstOrderHiddenMarkoveModel(pi,A,B)

for O,S in given_model.generate(3,5,2):
    print(" ".join((observations_index_label[o] + "/" + states_index_label[s]) for o,s in zip(O,S)))


trained_model = FirstOrderHiddenMarkoveModel()
trained_model.train(given_model.generate(3,10,100000))
assert  trained_model.similar(given_model)
trained_model.unLog() #将对数形式概率还原
